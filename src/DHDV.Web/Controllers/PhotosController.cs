        ...
        public async Task<IActionResult> Index(int albumId, string q = "", int page = 1, int pageSize = 24)
        {
            ...
            var items = await photos.OrderBy(p => p.Id)
                ...
                .Select(p => new PhotoItem {
                    Id=p.Id, Url=p.Url, ThumbUrl=p.ThumbUrl ?? p.Url, Caption=p.Caption
                }).ToListAsync();

            ViewData["AlbumId"] = albumId;
            ViewData["AlbumTitle"] = album.Title ?? ("Album #"+album.Id);
            ViewData["q"]=q; ViewData["Total"]=total; ViewData["Page"]=page; ViewData["PageSize"]=pageSize;
            return View(items);
        }
    }
    public class PhotoItem { public int Id{get;set;} public string Url{get;set;}=""; public string ThumbUrl{get;set;}=""; public string? Caption{get;set;} }
}
